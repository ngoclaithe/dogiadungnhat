...
export default function CartPage() {
  ...
  if (!ready || !authReady) {
    return (
      <Container className="py-10 sm:py-12">
        <div className="mx-auto max-w-md rounded-[2rem] border border-line bg-cream p-6 text-sm text-stone sm:p-8">
          Đang tải giỏ hàng...
        </div>
      </Container>
    );
  ...
  if (!user) {
    return (
      <Container className="py-10 sm:py-12">
        <div className="mx-auto max-w-md rounded-[2rem] border border-line bg-cream p-6 text-center sm:p-8">
          <h1 className="font-display text-3xl">Giỏ hàng</h1>
          <p className="mt-3 text-sm text-stone">
            Giỏ hàng được lưu theo từng tài khoản. Vui lòng đăng nhập để xem và mua hàng.
          </p>
          <Link
  ...
  if (!items.length) {
    return (
      <Container className="py-10 sm:py-12">
        <div className="mx-auto max-w-md rounded-[2rem] border border-line bg-cream p-6 text-center sm:p-8">
          <h1 className="font-display text-3xl">Giỏ hàng</h1>
          <p className="mt-3 text-sm text-stone">Chưa có sản phẩm trong giỏ.</p>
          <Link
            href="/"
            className="mt-6 block w-full rounded-full bg-ink py-3 text-sm font-semibold text-cream"
          >
